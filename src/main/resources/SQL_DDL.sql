...
DROP TABLE IF EXISTS primary_keys_identity CASCADE;
DROP TABLE IF EXISTS primary_keys_sequence CASCADE;
DROP SEQUENCE IF EXISTS primary_keys_sequence_generator CASCADE;
DROP TABLE IF EXISTS primary_keys_table CASCADE;
DROP TABLE IF EXISTS primary_keys_table_generator CASCADE;
DROP TABLE IF EXISTS  table_for_test_lock CASCADE;
...
--lock
CREATE TABLE table_for_test_lock
(
    id   BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name VARCHAR NOT NULL,
    version  BIGINT
...
--primary_keys_identity
CREATE TABLE primary_keys_identity
(
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY
);

--primary_keys_sequence
CREATE SEQUENCE primary_keys_sequence_generator
    START WITH 1
    INCREMENT BY 50;
--primary_keys_sequence
CREATE TABLE primary_keys_sequence
(
    id BIGINT DEFAULT nextval('primary_keys_sequence_generator') PRIMARY KEY
);

--primary_keys_table
CREATE TABLE primary_keys_table_generator
(
    pk    VARCHAR PRIMARY KEY,
    value BIGINT
);
--primary_keys_table
CREATE TABLE primary_keys_table
(
